...
def test_settings_threshold_validation(client):
    login = client.post("/api/auth/login", json={"username": "admin", "password": "admin123"})
    token = login.json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}

    bad_high = client.put("/api/settings", json={"cpu_threshold": 150.0}, headers=headers)
    assert bad_high.status_code == 422

    bad_low = client.put("/api/settings", json={"cpu_threshold": -10.0}, headers=headers)
