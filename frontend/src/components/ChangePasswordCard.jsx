      ...
      {newPassword && (
        <div className="space-y-1 pt-1">
          <div className="flex items-center justify-between text-xs font-bold">
            ...
            <span className={
              newPassword.length < 8 ? 'text-rose-600 dark:text-rose-400' :
              (newPassword.length >= 10 && /[A-Z]/.test(newPassword) && /[0-9]/.test(newPassword)) ? 'text-emerald-600 dark:text-emerald-400 font-black' : 'text-amber-600 dark:text-amber-400'
            }>
              {newPassword.length < 8 ? 'Too Short' :
               (newPassword.length >= 10 && /[A-Z]/.test(newPassword) && /[0-9]/.test(newPassword)) ? 'Strong (Secure)' : 'Medium'}
            </span>
          </div>
          <div className="w-full bg-slate-200 dark:bg-slate-900 h-1.5 rounded-full overflow-hidden flex">
            <div className={`h-full transition-all duration-300 ${
              newPassword.length < 8 ? 'w-1/3 bg-rose-500' :
              (newPassword.length >= 10 && /[A-Z]/.test(newPassword) && /[0-9]/.test(newPassword)) ? 'w-full bg-emerald-500' : 'w-2/3 bg-amber-500'
            }`}></div>
          </div>
        </div>
      )}
