...
export default function IOPanel({ live }) {
  if (!live) return null;
  const netIn = (live.network_recv_mbps ?? 0).toFixed(2);
  const netOut = (live.network_sent_mbps ?? 0).toFixed(2);
  const diskRead = (live.disk_read_mbps ?? 0).toFixed(2);
  const diskWrite = (live.disk_write_mbps ?? 0).toFixed(2);
  ...
  return (
    <div className="telemetry-panel rounded-2xl p-5">
      ...
      <Row icon={ArrowDown} label="Network In" value={`${netIn} MB/s`} color="#38d0e0" />
      <Row icon={ArrowUp} label="Network Out" value={`${netOut} MB/s`} color="#2bd97c" />
      <Row icon={HardDriveDownload} label="Disk Read" value={`${diskRead} MB/s`} color="#f5a623" />
      <Row icon={HardDriveUpload} label="Disk Write" value={`${diskWrite} MB/s`} color="#ff5470" />
    </div>
  );
}
